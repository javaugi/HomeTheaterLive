...
async def process_directory(
    ...
    print(f"Endpoints process_directory job_id={
          job_id}, request.directory_path={request.directory_path}")
    try:
...
def run_async(coro):
    print(f"Endpoints run_async {coro}")
...
async def process_uploaded_images(
    ...
    # Create job ID
    job_id = str(uuid.uuid4())
    print(f"Endpoints process_uploaded_images job_id={job_id}")
    try:
        # Parse settings
        try:
        ...
        print(f"Endpoints process_uploaded_images job_id={
              job_id}, saved_paths={len(saved_paths)}")

        status_data = ProcessStatusCreate(
            job_id=job_id,
            status=ProcessStatuses.pending,
            progress=10,
            message="Processing job {job_id} {len(saved_paths)} images...",
            created_at=datetime.utcnow()
        )
        ...
        db_status = ProcessStatusCRUD.get_by_job_id(db, job_id)
        print(f"Endpoints process_uploaded_images return db_status={
              db_status}")
        # return db_status - using the response model
        return ProcessStatusResponse.from_orm(db_status)
...
async def get_process_status(
    ...
    try:
        db_status = ProcessStatusCRUD.get_by_job_id(db, job_id)
        print(f"Endpoints get_process_status job_id={
              job_id}, db_status={db_status}")
        if not db_status:
            raise HTTPException(
                status_code=404, detail="Error get_process_status - Job not found")

        return db_status
    except Exception as e:
        raise HTTPException(
            status_code=404, detail=f"Error Job not found get_process_status {str(e)}")


@router.get("/video/download/{filename}", response_model=ProcessStatusResponse)
async def get_video_file(filename: str,
    ...
    # Create job ID
    job_id = str(uuid.uuid4())
    print(f"Endpoints get_video_file job_id={job_id}, filename={
          filename}, video_path={video_path}")
    status_data = ProcessStatusCreate(
        job_id=job_id,
...
async def download_video(filename: str):
    ...
    from app.core.video_processor import get_video_processor
    video_path = os.path.join(get_video_processor().output_dir, filename)
    print(f"Endpoints download_video filename={
          filename}, video_path={video_path}")

    if not os.path.exists(video_path):
        raise HTTPException(
            status_code=404, detail="Endpoints download_video Video not found")
...
async def save_video(filename: str):
    """Download video file"""
    from app.core.video_processor import get_video_processor
    video_path = os.path.join(get_video_processor().output_dir, filename)
    print(f"Endpoints save_video filename={filename}, video_path={video_path}")
    if not os.path.isfile(video_path):
        raise HTTPException(404, "Endpoints save_video Video not found")

    size = os.path.getsize(video_path)
    print(f"Endpoints save_video filename={
          filename}, - real size: {size} bytes ({size / 1024 / 1024:.2f} MB)")
    if size == 0:
        raise HTTPException(500, "Generated video file is empty on server")

    return FileResponse(
        video_path,
        media_type="video/mp4",
        filename=filename,
        headers={"X-Debug-Size": str(size)}  # optional for debugging
    )
...
async def process_video_task(job_id: str,
    ...
    try:
        ...
        print(f"Endpoints process_video_task job_id={
              job_id}, db_status={db_status}")
...
async def process_upload_task(job_id: str,
    ...
    try:
        ...
        print(f"Endpoints process_upload_task job_id={
              job_id}, db_status={db_status}")
...
async def create_video(
    ...
    # Create job ID
    job_id = str(uuid.uuid4())
    print(f"Endpoints create_video, job_id={
          job_id}, fps={fps}, files={len(files)}")
    try:
        # Create temp directory for uploaded files
        ...
        for file in files:
            ...
            saved_paths.append(file_path)
        print(f"Endpoints create_video, job_id={job_id}, fps={
              fps}, saved_paths={len(saved_paths)}")
        ...
        status_data = ProcessStatusCreate(
            ...
            status=ProcessStatuses.pending,
            progress=4,
            message=f"Endpoints create_video job_id {
                job_id} Uploading {len(files)} images ",
            video_path=temp_dir,
            created_at=datetime.utcnow()
        )
        print(f"Endpoints calling ProcessStatusCRUD.create, job_id={
              job_id}, saved_paths={len(saved_paths)}, status_data={status_data}")

        db_status = ProcessStatusCRUD.create(db, status_data)
        print(f"Endpoints create_video, job_id={job_id}, saved_paths={
              len(saved_paths)}, db_status={db_status}")
        ...
        print(
            f"Endpoints /videos/create status_code=500 db_status={db_status}, \n exception={str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def process_video_background(
    ...
    print(f"Endpoints process_video_background job_id={
          job_id}, fps={fps}, image_paths={len(image_paths)}")
    db = SessionLocal()           # ← fresh session per task
    try:
        # Update status
        db_status = ProcessStatusCRUD.update(
        ...
        print(f"Endpoints process_video_background exception={
              str(e)}, \n db_status={db_status}")
...
async def get_video_status(
    ...
    if not db_status:
        raise HTTPException(
            status_code=404, detail="Error get_video_status - Job not found")
