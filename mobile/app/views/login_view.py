...
class LoginView(toga.Box):
    ...
    async def login(self, widget):
        ...
        print(f"mobile/app/views/login.py Attempting login with username: {username}")
        ...
        if result.get("success"):
            print("mobile/app/views/login.py Login successful, switching to HomeView mobile/app/home_view.py")
            # Switch to HomeView
            self.app.main_window.content = self.home_view
            #from .. import views
            #self.app.main_window.content = views.get_home_view(self.app)
        else:
            self.error_label.text = f"mobile/app/views/login.py Login failed: {result.get('error', 'Unknown error')}"
